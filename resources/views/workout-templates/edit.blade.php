...
@section('content')
    <x-common.page-breadcrumb :pageTitle="'Edit Workout Template'" :items="[['label' => 'Users', 'url' => route('users.index')], ['label' => $workoutTemplate->plan->user->name, 'url' => route('users.show', $workoutTemplate->plan->user)], ['label' => $workoutTemplate->plan->name, 'url' => route('plans.show', $workoutTemplate->plan)], ['label' => $workoutTemplate->name, 'url' => route('workouts.show', $workoutTemplate)]]" />
    ...
    <x-common.component-card title="Edit Workout Template Information" :desc="'Update workout template details'">
        <form action="{{ route('workouts.update', $workoutTemplate) }}" method="POST">
            @csrf
            @method('PUT')
